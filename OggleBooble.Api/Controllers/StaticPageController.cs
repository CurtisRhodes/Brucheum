...
namespace OggleBooble.Api.Controllers
{
    ...
    public class StaticPageController : ApiController
    {
        ...
        private string GetStaticPageHeader(int folderId, string rootFolder)
        {
            ...
            using (var db = new MySqlDataContext.OggleBoobleMySqlContext())
            {
                ...
                StringBuilder staticPageHeader = new StringBuilder(
                    ...
                    "           <div id='breadcrumbContainer' class='breadCrumbArea'>" + GetBreadCrumbs(folderId) + "</div>\n" +
        ...
        private string GetBreadCrumbs(int folderId)
        {
            ...
            using (var db = new OggleBoobleMySqlContext())
            {
                ...
                //int breadcrumbCount = breadcrumbs.Count;

                breadCrumbString.Append("<a class='activeBreadCrumb' href='javascript:rtpe(\"HBX\"," +
                    folderId + ",\"" + thisFolder.RootFolder + "\")'>home  &#187</a>");


                for (int i = breadcrumbs.Count - 1; i >= 0; i--)
                //for (int i = 0; i < breadCrumbModel.BreadCrumbs.Count; i++)
                {
                    if (breadcrumbs[i].IsInitialFolder)
                    {
                        breadCrumbString.Append("<a class='inactiveBreadCrumb' " +
                        (breadcrumbs.Count - i) + "'," +
                        thisFolder.FolderName + "\",\"" +
                        breadcrumbs[i].FolderId + "\",\"" +
                        breadcrumbs[i].ParentId + "\",\"" +
                        thisFolder.RootFolder + "\"); forgetHomeFolderInfoDialog=false;' onmouseout='forgetHomeFolderInfoDialog=true;' " +
                        "onclick='showEitherModelorFolderInfoDialog()'>" +
                        breadcrumbs[i].FolderName.Replace(".OGGLEBOOBLE.COM", "") +
                        "</a>");
                    }
                    else
                    {
                        ...
                        breadCrumbString.Append("<a class='activeBreadCrumb'" +
                            "href='javascript:rtpe(\"BCC\"," + folderId + ",33," + breadcrumbs[i].FolderId + ")'>" +
                            breadcrumbs[i].FolderName.Replace(".OGGLEBOOBLE.COM", "") + " &#187</a>");
                    }
                }
            }
